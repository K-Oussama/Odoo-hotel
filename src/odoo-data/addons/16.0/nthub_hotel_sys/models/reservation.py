...
class Reservation(models.Model):
    ...
    @api.onchange('res_start_date', 'res_end_date', 'reservation_type')
    def _onchange_reservation_dates(self):
        # Calculate the domain to filter rooms that meet the conditions
        if self.res_start_date or self.res_end_date or self.reservation_type:
            self.room_id = False
            # domain = self._get_available_rooms_domain()
        return {'domain': {'room_id': self._get_available_rooms_domain()}}
